...
## 说明
了解 `Linux` 引导和启动过程，有助于增加对 `Linux` 的了解，日常工作中碰到系统启动相关问题也能知道是什么原因导致的。 本文主要说明使用 `GRUB2` 引导加载程序完成系统的 `boot 流程`以及 `systemd` 初始化系统执行的`startup 流程`。
...
## BIOS POST
`Linux` 启动过程的第一步实际上和 `Linux` 无关，这是引导过程的硬件部分，对于任何操作系统都是一样的。当计算机首次通电时，它会运行 `POST`（开机自检）。`POST` 是 `BIOS` 的一部分，其任务是确保计算机硬件正常运行。如果 `POST` 失败，计算机可能无法使用，因此引导过程不会继续。
...
任一 `GRUB` 的主要功能是将 `Linux` 内核加载到内存中并运行。两个 `GRUB（grub 和 grub2）` 版本的工作方式基本相同，并且具有相同的三个阶段。 虽然官方并没有使用此处 `GRUB2` 的三个阶段来说明 `GRUB2` 的工作流程，但是这样理解会比较方便。

### Stage 1
如 `BIOS POST` 部分所述，在 `POST` 结束时，`BIOS` 在连接的磁盘中搜索引导记录。引导记录通常位于`主引导记录 (MBR) `中，它将找到的{{< underline color="#ffdd00" content="第一个" >}}记录加载到内存中，然后开始执行开机。`引导代码（即 GRUB2 stage 1）`非常小，因为它必须与分区表一起放入硬盘驱动器上的第一个 `512` 字节扇区。在经典通用 `MBR` 中为实际引导代码分配的空间总量为 `446` 字节。阶段 1 的 `446` 字节文件名为 `boot.img`，不包含单独添加到引导记录的分区表。
...
这是`boot process`的结束。此时，`Linux` 内核和 `systemd` 正在运行，但无法为用户执行任何任务，因为当前没有任何服务在运行。
